...
def DieUser():
    ...
    while True:
        ...
        myfont = pygame.font.Font(
            FONT_DIR + 'NanumGothic.ttf', 20)      # 한글 폰트
        txt = myfont.render('Game Over',
                            True, (255-r, 255-g, 255-b))
...
def playGame():
    global monitor, ship, monster, missile, heart, heartImage, userStatus
    ...
    monster = pygame.image.load(IMAGES_DIR + random.choice(monsterImage))
    ...
    while True:
        ...
        print("남은 하트의 개수:", heart)
        for i in range(heart):  # 남은 하트의 개수를 보여준다
            paintEntity(heartImage, 220 + (30 * i), sheight - 38)
        ...
        if monsterX > swidth:
            if (heart < 2):
                print('Die')
                userStatus = UserStatusEnum.DIE.value
                return
            heart -= 1  # 우주괴물을 놓칠 때 마다 하트가 감소한다.
            monsterX = 0
            monsterY = random.randrange(0, int(swidth * 0.3))
            # 우주괴물 이미지를 무작위로 선택한다.
            monster = pygame.image.load(IMAGES_DIR +
                                        random.choice(monsterImage))
...
heart = 3  # 남은 하트 개수
heartImage = None
userStatus = UserStatusEnum.LIVE.value
...
ship = pygame.image.load(IMAGES_DIR + 'ship02.png')
shipSize = ship.get_rect().size

# 하트 이미지 준비
heartImage = pygame.image.load(IMAGES_DIR + 'heart.png')


# @기능 4-1 : 미사일 이미지를 추가한다.
missile = pygame.image.load(IMAGES_DIR + 'missile.png')

while True:
    if (userStatus == UserStatusEnum.LIVE.value):
        playGame()
    else:
        DieUser()
